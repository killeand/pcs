...
export default function MultiNumber({title, id, value, className, onChange, ...props}) {
        ...
        for (let i = 0; i < title.length; i++) {
            retval.push(
                <div key={`${formId}-section${i}`} className="mn-section">
                    ...
                    <input type="number" id={`${formId}-form${i}`} name={`${formId}-form${i}`} value={formValue[i]} className="mn-num" onChange={(e)=>ChangeValue(e,i)} />
                </div>
            );
        }
