...
export default function MultiNumber({ title, id, value, min, max, color, innerClass, outerClass, onChange }) {
    ...
    function RenderSections() {
        ...
        for (let i = 0; i < title.length; i++) {
            retval.push(
                <label key={`${formId}-section${i}`} htmlFor={`${formId}-form${i}`} className={`flex flex-grow flex-col [&>div]:first:rounded-tl-md [&>div]:last:rounded-tr-md [&>input]:first:rounded-bl-md [&>input]:last:rounded-br-md`}>
